...
#define DELETE_MAXDISTANCE [UIScreen mainScreen].bounds.size.width * 3/8
#define DELETE_MINDISTANCE [UIScreen mainScreen].bounds.size.width * 2/9
#define DELETE_SPEED 500

#ifdef ICAROUSEL_MACOS
#define MAX_VISIBLE_ITEMS 50
#else
#define MAX_VISIBLE_ITEMS 1000
#endif
...
@property (nonatomic, assign, getter = isGestureDragging) BOOL gestureDragging;
//记录当结束拖拽的时候卡片总位移距离，之后将其复位
@property (nonatomic, assign) CGFloat gestureDragDistance;
@property (nonatomic, strong) NSDate* gestureDragStartTime;
@property (nonatomic, weak) UIView *currentGestureView;
...
//    if (view == nil) {
//        for (UIView *sv in [[self.subviews reverseObjectEnumerator] allObjects]) {
//            CGPoint svPoint = [sv convertPoint:point fromView:self];
//            UIView *respondV = [sv hitTest:svPoint withEvent:event];
//            if (respondV != nil) {
//                return respondV;
//            }
//        }
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //TODO:[115浏览器][Bug][标签页][iPhone]点击右下角多窗口菜单后反应较缓慢
//    [view layoutIfNeeded];
...
//        CGFloat alpha = [self alphaForItemWithOffset:[self offsetForItemAtIndex:index]];
        ...
        // TODO:屏蔽alpha的if判断，不然在475行加上 fadeMin = -2.0之后会手势删除后排版错乱
//        if (alpha)
//        {
...
- (void)removeItemAtIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    if([self.delegate respondsToSelector:@selector(carousel:didRemoveItemAtIndex:animated:)]){
        [self.delegate carousel:self didRemoveItemAtIndex:index animated:YES];
    }
}
...
- (void)didPan:(UIPanGestureRecognizer *)panGesture
{
    if (_scrollEnabled && _numberOfItems)
    {
        switch (panGesture.state)
        {
            ...
            {
                NSInteger index = [self indexOfItemView:[self itemViewAtPoint:[panGesture locationInView:_contentView]]];
                if (index != NSNotFound && index < self.numberOfItems){
                    UIView *gestureView = [self itemViewAtIndex:index];
                    if (!self.currentGestureView) {
                        self.currentGestureView = gestureView;
                        self.gestureDragStartTime = [NSDate date];
                    }
                    //                    break;
                }
            ...
            case UIGestureRecognizerStateEnded:
            {
                ...
                //                {
                NSInteger index = [self indexOfItemView:[self itemViewAtPoint:[panGesture locationInView:_contentView]]];
                if ((index != NSNotFound && index < self.numberOfItems) && self.gestureDragging){
                    NSDate *gestureDragEndTime = [NSDate date];
                    NSTimeInterval deltaTime = [gestureDragEndTime timeIntervalSinceDate:self.gestureDragStartTime];
                    double speed = self.gestureDragDistance / deltaTime;
                    
                    if (fabs(self.gestureDragDistance) < DELETE_MINDISTANCE || (fabs(self.gestureDragDistance) < DELETE_MAXDISTANCE && fabs(speed) < DELETE_SPEED)) {
                        [UIView animateWithDuration:0.25 animations:^{
                            self.currentGestureView.layer.transform = CATransform3DIdentity;
                            self.currentGestureView.layer.opacity = 1;
                        }];
                    } else {
                        if([self indexOfItemView:self.currentGestureView] == index && (index != 0)) {
                            [self removeItemAtIndex:index animated:YES];
                        } else {
                            [UIView animateWithDuration:0.25 animations:^{
                                self.currentGestureView.layer.transform = CATransform3DIdentity;
                                self.currentGestureView.layer.opacity = 1;
                    ...
                    _startVelocity = 0.0;
                    self.gestureDragging = NO;
                    self.currentGestureView = nil;
                } else {
                    [UIView animateWithDuration:0.25 animations:^{
                        self.currentGestureView.layer.transform = CATransform3DIdentity;
                        self.currentGestureView.layer.opacity = 1;
                ...
                self.gestureDragDistance = 0;
                self.gestureDragStartTime = nil;
            }
            ...
            case UIGestureRecognizerStateChanged:
            {
                ...
                if ((fabs(translationPoint.x) > fabs(translationPoint.y) + 1) || self.gestureDragging == YES)
                {
                    self.gestureDragging = YES;
                    NSInteger index = [self indexOfItemView:[self itemViewAtPoint:[panGesture locationInView:_contentView]]];
                    if (index != NSNotFound && index < self.numberOfItems){
                        //                        UIView *gestureView = [self itemViewAtIndex:index];
                        //                        if (!self.currentGestureView) {
                        ...
                        self.gestureDragDistance += translationPoint.x;
                        [UIView animateWithDuration:0.1 animations:^{
                            self.currentGestureView.layer.transform = CATransform3DTranslate(self.currentGestureView.layer.transform, translationPoint.x, 0, 0);
                            self.currentGestureView.layer.opacity = 1 - fabs(self.gestureDragDistance) / DELETE_MAXDISTANCE;
                        }];
